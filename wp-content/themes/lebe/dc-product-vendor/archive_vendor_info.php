...
<div class="vendor_description_background wcmp_vendor_banner_template <?php echo $template_class; ?>">
		...
		<?php if ( apply_filters( 'wcmp_vendor_store_header_show_social_links', true, $vendor->id ) ) : ?>
            <div class="wcmp_social_profile">
				...
				<?php if ( $vendor_twitter_profile ) { ?> <a target="_blank"
                                                             href="<?php echo esc_url( $vendor_twitter_profile ); ?>"><i
                            class="wcmp-font ico-twitter-icon"></i></a><?php } ?>
				<?php if ( $vendor_linkdin_profile ) { ?> <a target="_blank"
                                                             href="<?php echo esc_url( $vendor_linkdin_profile ); ?>"><i
                            class="wcmp-font ico-linkedin-icon"></i></a><?php } ?>
				...
				<?php if ( $vendor_youtube ) { ?> <a target="_blank" href="<?php echo esc_url( $vendor_youtube ); ?>"><i
		...
		if ( $template_class == 'template1' ) {
            ...
            <div class="vendor_description">
                <div class="vendor_img_add">
                    <div class="img_div"><img height="400" width="200"
                                                       src="<?php echo esc_url( $profile ); ?>"/></div>
                    <div class="vendor_address">
						...
						<?php if ( ! empty( $location ) && $vendor_hide_address != 'Enable' ) { ?><p
                                class="wcmp_vendor_detail"><i
                                    class="wcmp-font ico-location-icon"></i><label><?php echo apply_filters( 'vendor_shop_page_location', $location, $vendor_id ); ?></label>
                            </p><?php } ?>
	...
	if ( $template_class != 'template1' ) {
        ...
        <div class="vendor_description">
            <div class="vendor_img_add">
                <div class="img_div"><img height="400" width="200" src=<?php echo esc_url( $profile ); ?>/></div>
                <div class="vendor_address">
                    <p class="wcmp_vendor_name"><?php echo esc_html( $vendor->page_title ); ?></p>
					<?php do_action( 'before_wcmp_vendor_information', $vendor_id ); ?>
                    <div class="wcmp_vendor_rating">
					...
					<?php if ( ! empty( $mobile ) && $vendor_hide_phone != 'Enable' ) { ?><p class="wcmp_vendor_detail">
                        <i class="wcmp-font ico-call-icon"></i><label><?php echo apply_filters( 'vendor_shop_page_contact', $mobile, $vendor_id ); ?></label>
                        </p><?php } ?>
					<?php if ( ! empty( $email ) && $vendor_hide_email != 'Enable' ) { ?><a
                        href="mailto:<?php echo apply_filters( 'vendor_shop_page_email', $email, $vendor_id ); ?>"
                        class="wcmp_vendor_detail"><i
