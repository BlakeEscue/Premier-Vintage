...
if ( class_exists( 'WooCommerce' ) && isset( $post_meta_data['lebe_product_options'] ) ) {
	if ( ! empty( $post_meta_data['lebe_product_options'] ) ) {
		$product_style = '1';
		$product_ids   = $post_meta_data['lebe_product_options'];
		$query_args    = array(
			'post_type' => 'product',
		...
		if ( $products->have_posts() ) { ?>
            <div class="single-post-products-carousel-wrap product-grid">
                <h3 class="single-post-products-carousel-title text-center"><?php esc_attr_e( 'Special Products', 'lebe' ); ?></h3>
                <div class="single-post-products-carousel owl-carousel equal-container nav-center"
