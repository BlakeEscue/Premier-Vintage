...
if ( ! class_exists( 'Lebe_MegaMenu' ) ) {
	class Lebe_MegaMenu {
		...
		function register_scripts( $hook ) {
			...
			wp_enqueue_style( 'lebe-megamenu', get_template_directory_uri() . '/framework/includes/megamenu/css/megamenu.css' );
			...
			wp_enqueue_media();
			wp_enqueue_script( 'lebe-megamenu', get_template_directory_uri() . '/framework/includes/megamenu/js/megamenu.js', array( 'jquery' ), '1.0.0', true );
			...
			wp_localize_script( 'lebe-megamenu', 'lebe_ajax_backend', array(
					'ajaxurl'  => esc_url( admin_url( 'admin-ajax.php' ) ),
					'security' => wp_create_nonce( 'lebe_ajax_backend' ),
				)
			);
		}
		...
		public function set_fonts() {
			$this->lits_fonts = array(
				array(
					'title' => esc_html__( 'Font Awesome', 'lebe' ),
					'fonts' => array(
						'fa fa-car',
						'fa fa-map-marker',
						'fa fa-phone',
		...
		public function addShortcodesCustomCss_megamenu() {
			...
			if ( $posts_array ) {
				...
				foreach ( $posts_array as $post ) {
					...
					$shortcodes_custom_css .= get_post_meta( $post->ID, '_lebe_shortcode_custom_css', true );
				...
				if ( ! empty( $shortcodes_custom_css ) ) {
					...
					wp_add_inline_style( 'lebe-custom', $shortcodes_custom_css );
				}
			}
		}
