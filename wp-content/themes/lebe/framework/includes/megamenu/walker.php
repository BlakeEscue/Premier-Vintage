<?php
/**
 * Class Name: lebe_bootstrap_navwalker
 ...
 */
if ( ! class_exists( 'Lebe_navwalker' ) ) {
	class Lebe_navwalker extends Walker_Nav_Menu {
		/**
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$this->megamenu_img_ico              = get_post_meta( $item->ID, '_menu_item_megamenu_img_ico', true );
			...
			if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
				...
				if ( $this->megamenu_item_icon_type != "none" ) {
					// Icon image
					if ( $this->megamenu_item_icon_type == "image" ) {
						if ( $this->megamenu_img_ico ) {
							$imgicon = wp_get_attachment_image( intval( $this->megamenu_img_ico ), 'full', null, array( 'class' => 'image_icon_1' ) );
						...
						if ( $this->megamenu_img_icon ) {
							...
							if ( $imgicon ) {
								...
								if ( $imgicon_ext == 'svg' ) {
									...
									$item_output .= '<span class="logo">' . lebe_img_output( $imgicon_arg, 'image_icon_1' ) . '</span>';
								} else {
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'manage_options' ) ) {
				...
				$fb_output .= '>';
				$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">' . esc_html__( 'Add a menu', 'lebe' ) . '</a></li>';
				$fb_output .= '</ul>';
				if ( $container ) {
