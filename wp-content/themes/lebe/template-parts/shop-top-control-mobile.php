<?php if ( ! is_product() ) { ?>
	...
	$owl_item_close       = ''; // </div>';
	$list_categories_html = $owl_item_open . '<span data-cat_slug="" class="active all-cats cat-item"><a href="' . esc_url( $shop_page_url ) . '">' . esc_html__( 'All', 'lebe' ) . '</a></span>';
	$total_cats_show      = 1;
        ...
        <div class="part-wrap part-filter-wrap">
			<?php if ( class_exists( 'PrdctfltrInit' ) ) { ?>
                <div class="actions-wrap">
                	...
                	<div class="action-mini">
                    	<a class="filter-toggle" href="javascripti:void(0);"><?php esc_html_e( 'Filter', 'lebe' ); ?></a>
                    </div>
                </div>
			<?php } ?>
        </div>
