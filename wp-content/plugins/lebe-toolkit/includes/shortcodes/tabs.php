...
if ( !class_exists( 'Lebe_Shortcode_Tabs' ) ) {
	class Lebe_Shortcode_Tabs extends Lebe_Shortcode
	{
		/**
		 * Shortcode name.
		 *
		...
		{
			$atts = function_exists( 'vc_map_get_attributes' ) ? vc_map_get_attributes( 'lebe_tabs', $atts ) : $atts;
			// Extract shortcode parameters.
			extract(
				shortcode_atts(
					$this->default_atts,
			...
			$css_class = 'lebe-tabs ' . $atts[ 'el_class' ] . ' ' . $atts[ 'style' ] . ' ' . $atts[ 'tabs_custom_id' ] . ' ' . $atts['animate_on_scroll'];
                        ...
                        <ul class="tab-link">
							...
							<?php foreach ( $sections as $section ): ?>
								...
								if ( $type_icon == 'fontflaticon' ) {
									$class_icon = isset( $section[ 'icon_lebecustomfonts' ] ) ? $section[ 'icon_lebecustomfonts' ] : '';
								} else {
			...
			return apply_filters( 'Lebe_Shortcode_tabs', $html, $atts, $content );
		}
	}
}