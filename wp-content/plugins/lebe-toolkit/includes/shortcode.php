<?php
if ( ! class_exists( 'Lebe_Shortcode' ) ) {
	class Lebe_Shortcode {
		/**
		 * Shortcode name.
		 *
		 * @var  string
		 ...
		 * @var  string
		 */
		protected $metakey = '_Lebe_Shortcode_custom_css';
		
		public function __construct() {
			if ( ! empty( $this->shortcode ) ) {
				// Add shortcode.
				add_shortcode( "lebe_{$this->shortcode}", array( &$this, 'output_html' ) );
		...
		public function replace_post( $post_id ) {
			...
			if ( $post ) {
				if ( has_shortcode( $post->post_content, "lebe_{$this->shortcode}" ) ) {
					
					$post->post_content = preg_replace_callback(
						'/(' . $this->shortcode . '_custom_id)="[^"]+"/',
						'Lebe_Shortcode_replace_post_callback',
						$post->post_content
					);
...
if ( ! function_exists( 'Lebe_Shortcode_replace_post_callback' ) ) {
	
	function Lebe_Shortcode_replace_post_callback( $matches ) {
		// Generate a random string to use as element ID.
		$id = 'lebe_custom_css_' . mt_rand();
...
foreach ( $shortcodes as $shortcode ) {
	...
	if ( is_file( LEBE_TOOLKIT_PATH . '/includes/shortcodes/' . $shortcode . '.php' ) ) {
		
		include_once LEBE_TOOLKIT_PATH . '/includes/shortcodes/' . $shortcode . '.php';
	}
...
if ( ! function_exists( 'Lebe_Shortcode_print_inline_css' ) ) {
	function Lebe_Shortcode_print_inline_css() {
		// Get all custom inline CSS.
		if ( is_singular() ) {
			$post_custom_css = get_post_meta( get_the_ID(), '_Lebe_Shortcode_custom_css', true );
			$inline_css[]    = $post_custom_css;
			$inline_css      = apply_filters( 'lebe-shortcode-inline-css', $inline_css );
			if ( count( $inline_css ) ) {
				echo '<style id="lebe-toolkit-inline" type="text/css">' . trim( implode( ' ', $inline_css ) ) . "</style>\n";
			}
		}
...
add_action( 'wp_head', 'Lebe_Shortcode_print_inline_css', 99999 );

/** Loadmore Product Ajax  **/

/* Loadmore Product */
add_action( 'wp_ajax_lebe_loadmore_product', 'lebe_loadmore_product' );
add_action( 'wp_ajax_nopriv_lebe_loadmore_product', 'lebe_loadmore_product' );
function lebe_loadmore_product() {
	$response        = array(
		'html'    => '',
		'message' => '',
		'success' => 'no',
...
// AJAX Tabs
function lebe_detect_shortcode( $id, $tab_id ) {
	$post = get_post( $id );
...
/* AJAX TABS */
if ( ! function_exists( ( 'lebe_ajax_tabs' ) ) ) {
	function lebe_ajax_tabs() {
		$response   = array(
			'html'    => '',
			'message' => '',
			'success' => 'no',
		...
		$shortcode  = lebe_detect_shortcode( $id, $section_id );
	...
	// TABS ajaxify update
	add_action( 'wp_ajax_lebe_ajax_tabs', 'lebe_ajax_tabs' );
	add_action( 'wp_ajax_nopriv_lebe_ajax_tabs', 'lebe_ajax_tabs' );
}
// GET REVO SLIDE /
