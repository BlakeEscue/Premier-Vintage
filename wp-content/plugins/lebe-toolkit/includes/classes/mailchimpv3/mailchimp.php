...
if ( !class_exists( 'FamiMailChimp' ) ) {
	class FamiMailChimp
	{
		...
		public function __construct()
		{
		    ...
		    $mail_chimp_options = get_option('lebe_mailchimp_option', $mail_chimp_options_default);
		...
		{
			wp_enqueue_script( 'fami-mailchimp', LEBE_TOOLKIT_URL . '/includes/classes/mailchimpv3/mailchimp.js', array( 'jquery' ), '1.0', true );
		...
		public function submit_mailchimp_via_ajax()
		{
			...
			if ( class_exists( 'MCAPI' ) ) {
				...
				if ( $api->subscribe( $list_id, $email, $merge_vars, 'html', false ) === true ) {
		...
		{
			$lists   = array();
			$api_key = lebe_toolkit_option( 'api_key', '' );
