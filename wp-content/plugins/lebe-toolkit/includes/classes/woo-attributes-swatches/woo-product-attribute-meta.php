...
if ( ! class_exists( 'Lebe_Attribute_Product_Meta' ) ) {
	class Lebe_Attribute_Product_Meta {
		...
		public function product_attributes_type_selector( $types ) {
			...
			return array_merge( $types, $lebe_types );
		}
		...
		public function add_attribute_fields() {
            ...
            <div class="form-field" style="display: none;">
                <label><?php esc_html_e( 'Size Box', 'lebe' ); ?></label>
                <input type="text" name="attribute_size" id="attribute_size" value="30x30">
                <p class="description"><?php esc_html_e( 'Determines Size in this attribute.{width}x{height}', 'lebe' ); ?></p>
            </div>
		...
		public function edit_attribute_fields() {
            ...
            <tr class="form-field form-required" <?php if ( $attribute->attribute_type == 'select' ): ?> style="display: none;" <?php endif; ?>>
                <th scope="row" valign="top"><label><?php _e( 'Size Box', 'lebe' ); ?></label></th>
                <td>
		...
		public function add_attr_field() {
            ...
            <div class="form-field term-display-type-wrap">
                <label for="display_type"><?php _e( 'Display type', 'lebe' ); ?></label>
                <select name="<?php echo esc_attr( $id_type ); ?>" id="product_attribute_type_id">
                    <option value="-1"><?php echo esc_html__( 'None', 'lebe' ); ?></option>
                    <option value="color"><?php echo esc_html__( 'Color', 'lebe' ); ?></option>
                    <option value="photo" selected><?php echo esc_html__( 'Photo', 'lebe' ); ?></option>
                    <option value="label"><?php echo esc_html__( 'Label', 'lebe' ); ?></option>
                </select>
                <script type="text/javascript">
            ...
            <div class="form-field term-display-type-wrap swatch photo">
                <label><?php _e( 'Thumbnail', 'lebe' ); ?></label>
                ...
                <div style="line-height: 60px;">
                    ...
                    <button type="button"
                            class="upload_image_button button"><?php _e( 'Upload/Add image', 'lebe' ); ?></button>
                    <button type="button"
                            class="remove_image_button button"><?php _e( 'Remove image', 'lebe' ); ?></button>
                </div>
                <script type="text/javascript">
                    ...
                    jQuery(document).on('click', '.upload_image_button', function (event) {
                        ...
                        file_frame = wp.media.frames.downloadable_file = wp.media({
                            title: '<?php _e( 'Choose an image', 'lebe' ); ?>',
                            button: {
                                text: '<?php _e( 'Use image', 'lebe' ); ?>'
                            },
                            multiple: false
                        });
		...
		public function edit_attr_field( $term, $taxonomy ) {
			...
			$id_color    = "product_attribute_meta[{$this->meta_key}][color]";
			$id_photo    = "product_attribute_meta[{$this->meta_key}][photo]";
			$swatch_term = new Lebe_Term( $this->meta_key, $term->term_id, $taxonomy, false );
            ...
            <tr class="form-field swatch color <?php echo $swatch_term->get_type() == 'color' ? 'active' : ''; ?>">
                <th scope="row" valign="top"><label><?php _e( '', 'lebe' ); ?></label></th>
                <td>
		...
		public function product_attr_columns( $columns ) {
			...
			$new_columns['thumb'] = __( 'Image', 'lebe' );
		...
		public function product_attr_column( $columns, $column, $id ) {
			if ( 'thumb' === $column ) {
				$swatch_term = new Lebe_Term( $this->meta_key, $id, $this->taxonomy, false );
