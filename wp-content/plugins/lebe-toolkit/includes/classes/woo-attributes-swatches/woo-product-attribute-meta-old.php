<?php
if ( ! class_exists( 'Lebe_Attribute_Product_Meta' ) ) {
	class Lebe_Attribute_Product_Meta {
		public $screen;
		public $taxonomy;
		public $meta_key;
		 ...
		 *
		 * @return Lebe_Attribute_Product_Meta
		 */
...
//			if ( is_product() ) {
//				wp_enqueue_script( 'woo-attributes-swatches', LEBE_TOOLKIT_URL . 'includes/classes/woo-attributes-swatches/woo-attribute.js', array( 'jquery' ), '1.0', true );
//			}
		...
		public function product_attributes_type_selector( $types ) {
			$famishop_types = array(
				'box_style' => esc_html__( 'Box Style', 'lebe' ),
			);
		...
		public function fami_woocommerce_product_option_terms( $attribute_taxonomy, $i ) {
			...
			<?php if ( 'box_style' === $attribute_taxonomy->attribute_type || 'list' === $attribute_taxonomy->attribute_type ) : ?>
                <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'lebe' ); ?>"
                ...
                <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'lebe' ); ?></button>
                <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'lebe' ); ?></button>
                <button class="button fr plus add_new_attribute"><?php esc_html_e( 'Add new', 'lebe' ); ?></button>
			<?php endif; ?>
			<?php
		}
		...
		public function woocommerce_add_attribute_thumbnail_field() {
            ...
            <div class="form-field ">
                <label for="product_attribute_type_<?php echo $this->meta_key; ?>"><?php esc_html_e( 'Type', 'lebe' ) ?></label>
                <select name="product_attribute_meta[<?php echo $this->meta_key; ?>][type]"
                    ...
                    <option value="-1"><?php esc_html_e( 'None', 'lebe' ) ?></option>
                    <option value="color"><?php esc_html_e( 'Color', 'lebe' ) ?></option>
                    <option value="photo"><?php esc_html_e( 'Photo', 'lebe' ) ?></option>
                    <option value="label"><?php esc_html_e( 'Label', 'lebe' ) ?></option>
                </select>
            ...
            <div class="form-field swatch-field field-color section-color-swatch"
                ...
                <div id="swatch-color" class="<?php echo sanitize_title( $this->meta_key ); ?>-color">
                    <label><?php esc_html_e( 'Color', 'lebe' ); ?></label>
            ...
            <div class="form-field swatch-field field-photo" style="overflow:visible;display:none;">
                <div id="swatch-photo" class="<?php echo sanitize_title( $this->meta_key ); ?>-photo">
                    <label><?php esc_html_e( 'Thumbnail', 'lebe' ); ?></label>
                    ...
                    <div style="line-height:60px;">
                        ...
                        <button type="submit"
                                class="upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'lebe' ); ?></button>
                        <button type="submit"
                                class="remove_image_button button"><?php esc_html_e( 'Remove image', 'lebe' ); ?></button>
                    </div>
                    <script type="text/javascript">
		...
		public function woocommerce_edit_attributre_thumbnail_field( $term, $taxonomy ) {
			global $woocommerce;
			$swatch_term = new Lebe_Term( $this->meta_key, $term->term_id, $taxonomy, false, $this->image_size );
			$image       = '';
			?>
            <tr class="form-field ">
                <th scope="row" valign="top"><label><?php esc_html_e( 'Type', 'lebe' ); ?></label></th>
                <td>
                    <select name="product_attribute_meta[<?php echo $this->meta_key; ?>][type]"
                            id="product_attribute_swatchtype_<?php echo $this->meta_key; ?>" class="postform">
                        <option <?php selected( 'none', $swatch_term->get_type() ); ?>
                                value="-1"><?php esc_html_e( 'None', 'lebe' ); ?></option>
                        <option <?php selected( 'color', $swatch_term->get_type() ); ?>
                                value="color"><?php esc_html_e( 'Color', 'lebe' ); ?></option>
                        <option <?php selected( 'photo', $swatch_term->get_type() ); ?>
                                value="photo"><?php esc_html_e( 'Photo', 'lebe' ); ?></option>
                        <option <?php selected( 'label', $swatch_term->get_type() ); ?>
                                value="label"><?php esc_html_e( 'Label', 'lebe' ); ?></option>
                    </select>
                    <script type="text/javascript">
                        jQuery(document).ready(function ($) {
            ...
            <tr class="form-field swatch-field swatch-field-color section-color-swatch"
                ...
                <th scope="row" valign="top"><label><?php esc_html_e( 'Color', 'lebe' ); ?></label></th>
                <td>
                    <div id="swatch-color" class="<?php echo sanitize_title( $this->meta_key ); ?>-color">
                        <div id="product_attribute_color_<?php echo $this->meta_key; ?>_picker" class="colorSelector">
                            <div></div>
            ...
            <tr class="form-field swatch-field swatch-field-photo" style="overflow:visible;<?php echo $style; ?>">
                <th scope="row" valign="top"><label><?php esc_html_e( 'Photo', 'lebe' ); ?></label></th>
                <td>
                    <div id="product_attribute_thumbnail_<?php echo $this->meta_key; ?>"
                         style="float:left;margin-right:10px;">
		...
		public function woocommerce_product_attribute_columns( $columns ) {
			...
			$new_columns[ $this->meta_key ] = esc_html__( 'Thumbnail', 'lebe' );
			unset( $columns['cb'] );
		...
		public function woocommerce_product_attribute_column( $columns, $column, $id ) {
			if ( $column == $this->meta_key ) :
				$swatch_term = new Lebe_Term( $this->meta_key, $id, $this->taxonomy, false, $this->image_size );
		...
		public function wc_variation_attribute_options( $html = '', $args = '' ) {
			...
			$args                    = wp_parse_args( apply_filters( 'woocommerce_dropdown_variation_attribute_options_args', $args ), array(
				                                                                                                                         ...
				                                                                                                                         'id'               => '',
				                                                                                                                         'class'            => '',
				                                                                                                                         'show_option_none' => __( 'Choose an option', 'lebe' ),
			                                                                                                                         )
			);
			...
			$show_option_none_text = $args['show_option_none'] ? $args['show_option_none'] : __( 'Choose an option', 'lebe' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
					...
					foreach ( $terms as $term ) {
						if ( in_array( $term->slug, $options ) ) {
							...
							if ( function_exists( 'lebe_toolkit_resize_image' ) ) {
								$image_thumb = lebe_toolkit_resize_image( $data_photo, null, $attribute_swatch_width, $attribute_swatch_height, true, true, false );
		...
		public function wc_loop_variation_attribute_options() {
			global $product;
			$products_loop_attributes_display = lebe_get_option( 'products_loop_attributes_display', array() );
			...
			if ( $product->get_type() == 'variable' ) {
				...
				foreach ( $available_variations as $available_variation ) {
					$image_variable                            = lebe_toolkit_resize_image( $available_variation['image_id'], null, $width, $height, true, true, false );
		...
		public function get_product_attribute( $attribute ) {
			...
			try {
				...
				if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_id', __( 'A product attribute with the provided ID could not be found', 'lebe' ), 404 );
				}
...
new Lebe_Attribute_Product_Meta();